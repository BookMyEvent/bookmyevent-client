...
function EditEvent() {
  ...
  async function fetchDept() {
    ...
    for (let i of dept) {
      if (event.target.includes(i)) temp[i] = true;
      else temp[i] = false;
    }
    if (event.target.includes("All")) temp["All"] = true;
  ...
  async function validateData(evt) {
    ...
    switch (event.session) {
      case "FN":
        if (
          Number(new Date(event.endTime).getHours()) === 12 &&
          Number(new Date(event.endTime).getMinutes()) > 0
        ) {
    ...
    if (
      Number(new Date(event.startTime).getHours()) >
      Number(new Date(event.endTime).getHours())
    ) {
    ...
    if (
      ...
      Number(event.startTime.split(":")[1]) >
        Number(new Date(event.endTime).getMinutes())
    ) {
      generateDangerAlert("Invalid Program Timings");
      return;
    }
    ...
    <div className="form-container">
      ...
      <div className="card form">
        ...
        <div className="form-body">
              ...
              return (
                ...
                >
                  <input
                    ...
                    key={index}
                    name="Session"
                    checked={event.session === item ? true : false}
                  />
                  <label
          ...
          {event.venue === "OTHERS**" && (
            <div className="other-event-group">
              <input
                ...
                value={event.venueName && event.venueName}
                onChange={(evt) => {
              ...
              return (
                <span className="checkbox-item">
                  <input
                    ...
                    type="checkbox"
                    defaultChecked={audienceType[item] && true}
                    disabled={disable}
                    onChange={() => {
          ...
          <div className="time-group">
              ...
              <input
                ...
                value={`${
                  new Date(event.startTime).getHours() < 10
                    ? `0${new Date(event.startTime).getHours()}`
                    : new Date(event.startTime).getHours()
                }:${
                  new Date(event.startTime).getMinutes() < 10
                    ? `0${new Date(event.startTime).getMinutes()}`
                ...
                onChange={(evt) => {
                  ...
                  setEvent({
                    ...
                    startTime: new Date(
                      ...
                      temp.getMonth(),
                      temp.getDate(),
                      evt.target.value.slice(0, 2),
                      evt.target.value.slice(3, 5)
                    ),
                  });
                }}
                required
            ...
            <div className="time-group-input">
              ...
              <input
                ...
                disabled={disable}
                value={`${
                  new Date(event.endTime).getHours() < 10
                    ? `0${new Date(event.endTime).getHours()}`
                    : new Date(event.endTime).getHours()
                }:${
                  new Date(event.endTime).getMinutes() < 10
                    ? `0${new Date(event.endTime).getMinutes()}`
          ...
          {/* Submit Button */}
          <button className="btn btn-primary submit-btn" onClick={validateData}>
            UPDATE
          </button>
        </div>
      </div>
